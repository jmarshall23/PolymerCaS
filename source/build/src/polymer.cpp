...
// jmarshall - CaS implementation
GLuint casProgram;
GLuint casRenderTexture = 0;

const char *Polymer_CasShader_Vertex = {
	"#version 120\n"
...
const char *Polymer_PostCaSShader_Pixel = {
"#version 140\n"
"varying vec4 texcoords;\n"
...
/*
====================
Polymer_CopyFramebuffer
====================
*/
void Polymer_CopyFramebuffer(GLuint texId, int x, int y, int imageWidth, int imageHeight) {
...
void Polymer_DrawFullscreenPass(GLuint program, GLuint texId, int width, int height)
{
...
bool IsCasEnabled(void);
void Polymer_RenderCaSPass(void)
{
	if (!IsCasEnabled())
		return;
	...
	Polymer_CopyFramebuffer(casRenderTexture, 0, 0, xdim, ydim);
	Polymer_DrawFullscreenPass(casProgram, casRenderTexture, xdim, ydim);
}

void Polymer_CompileShader(GLuint shader)
{
	...
	if (infologLength > 1) {
		...
		{
			OSD_Printf("FATAL ERROR: %s\n", infoLog);
			exit(0);
		}
		else
		{
			OSD_Printf("While compiling CaS shader...\n");
			OSD_Printf("%s\n", infoLog);
		}
		delete infoLog;
	}
...
void Polymer_CasInit(void) {
	...
	glShaderSource(casVertexShader, 1, &Polymer_CasShader_Vertex, NULL);
	glShaderSource(casPixelShader, 1, &Polymer_PostCaSShader_Pixel, NULL);

	Polymer_CompileShader(casVertexShader);
	Polymer_CompileShader(casPixelShader);
